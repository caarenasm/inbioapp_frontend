...
export const extractErrorMessagesFromErrorResponse = (errorResponse: HttpErrorResponse) => {
  ...
  if (errorResponse.error) {
    ...
    if (errorResponse.error.message) {
      ...
      for (const property in errorResponse.error.message) {

        if (errorResponse.error.message.hasOwnProperty(property)) {

          // 6 - Extract it's array of errors
          const propertyErrors: Array<string> = errorResponse.error.message[property];
